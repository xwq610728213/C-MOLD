...
public class JenaStreamGeneratorLarge {
    ...
    public JenaStreamGeneratorLarge() {
    ...
    public static String getPREFIX() {
        return JenaStreamGeneratorLarge.PREFIX;
    }
    ...
    private void generateDataAndAddToStream(DataStream<Graph> stream, long ts) {
        ...
        int property_cardinality = 100;

        Node object_concept = NodeFactory.createURI(PREFIX + "Object");

        Node[] concepts = new Node[property_cardinality];
        for(int i = 0; i < property_cardinality; i++){
            concepts[i] = NodeFactory.createURI(PREFIX + "Concept" + i);
        }

        Node[] properties = new Node[property_cardinality];
        for(int i = 0; i < property_cardinality; i++){
            properties[i] = NodeFactory.createURI(PREFIX + "property" + i);
        }

        Node[] instances = new Node[property_cardinality];
        for(int i = 0; i < property_cardinality; i++){
            instances[i] = NodeFactory.createURI(PREFIX + "instance" + i);
        }
        ...
        //chance has property0
        Random rand = new Random();
        if(rand.nextInt(100) < 50){
            graph.add(object, properties[0], instances[0]);
            graph.add(instances[0], a, concepts[0]);
        }

        //chance has property1
        if(rand.nextInt(100) < 50){
            graph.add(object, properties[1], instances[1]);
            graph.add(instances[1], a, concepts[1]);
        }


        for(int i = 2; i < property_cardinality; i++){
            graph.add(object, properties[i], instances[i]);
            graph.add(instances[i], a, concepts[i]);
        }
